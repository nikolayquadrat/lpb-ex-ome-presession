...
Obtain the GRCh38 reference assembly with ALT contigs (Homo_sapiens_assembly38.fasta) and its FAI/dict indices from the Broad Institute's public Google Cloud bucket (gcp-public-data--broad-references/hg38/v0). Retrieve known-sites VCFs for GATK BQSR — HapMap 3.3, the Mills and 1000 Genomes gold-standard indels, and dbSNP 138 — from the same source.

#### 2. Functional annotation infrastructure
Download Ensembl VEP cache release 112 from ftp.ensembl.org. VEP plugin source code to be cloned from two repositories: the official Ensembl VEP_plugins repository pinned to release/115 (required for dbNSFP v5 column-layout compatibility), and the LOFTEE plugin from the konradjk/loftee repository on the grch38 branch. To accommodate VEP's single-directory plugin-loading model, all .pm files from both repositories should be copied into a single canonical directory (plugins/flat/); copies (rather than symlinks) avoid Singularity bind-mount path-resolution failures.
...
Adapter and quality trimming is performed with fastp 0.23.4 using default Illumina-adapter detection. Trimmed reads are aligned to GRCh38 (with ALT contigs) using BWA-MEM2 v2.2.1 (rule `r02a_bwamem2_align`), with read-group tags injected for sample identification. Alignments are coordinate-sorted and indexed with samtools 1.19. PCR/optical duplicates are marked using GATK MarkDuplicatesSpark (GATK 4.5.0.0), which performs sorting and duplicate marking in a single Spark-parallelized pass.
...
Per-sample GVCFs are produced by GATK HaplotypeCaller in -ERC GVCF mode. The cohort GVCFs are imported into a per-cohort GenomicsDB datastore (`r06_genomicsdbimport`), and joint genotyping is performed across all samples by GATK GenotypeGVCFs (`r07_genotypegvcfs`). Joint calling is chosen over per-sample calling to share evidence at borderline-quality sites; In the study, SNV and indel cohort sizes (n=32) below the VQSR threshold made hard filtering preferable to model-based recalibration.

#### 4. Hard filtering
Following GATK best practices for small cohorts, SNVs and indels are extracted and filtered separately. SNV filters: QD < 2.0, FS > 60.0, MQ < 40.0, MQRankSum < -12.5, ReadPosRankSum < -8.0, SOR > 3.0. Indel filters: QD < 2.0, FS > 200.0, ReadPosRankSum < -20.0, SOR > 10.0. Filtered SNVs and indels are merged and PASS-filtered (`r08e_merge_and_pass`).

#### 5. Normalization
The cohort VCF is left-aligned and multiallelics are split using `bcftools norm -m -any --check-ref w` against the reference FASTA (rule `r09_normalize`).

#### 6. Internal artifact filtering
*this step requires an exome panel, at least a couple dozen samples*<br>
Four r09 rules implement relatedness-aware artifact filtering. PLINK2 (v2.00a5.10) with autosome restriction and MAF/genotype-rate filters (`--maf 0.05 --geno 0.05 --hwe 1e-6`) produced a BED file from the cohort VCF (`r09b_make_plink_bed`); KING-format kinship coefficients are computed via `--make-king-table` (`r09c_kinship_table`). A custom Python helper (*scripts/lpb-exome-prioritisation-pick-family-representatives.py*) performed connected-component analysis on related-pair edges (KING kinship ≥ 0.0442, the third-degree-relative threshold), retaining one lexicographically-first representative per family (`r09d_pick_representatives`). The cohort VCF is then subset to representatives and re-tagged with bcftools +fill-tags AC,AN; sites with AC ≥ 2 among representatives that are absent or rare (AF < 0.001) in gnomAD v4.1 are flagged as artifact-suspect tuples (`r09e_artifact_blacklist`). This procedure removes recurrent batch / mapping / sample-prep artifacts that gnomAD-AF filtering alone cannot detect, while avoiding false positives from variants shared across related samples.

#### 7. Per-sample VCF extraction and annotation
The cohort VCF is demultiplexed into per-sample VCFs by bcftools 1.19 with private-variant retention (`r10_per_sample_vcf`).

**Functional annotation**. The cohort VCF is annotated by Ensembl VEP 112 (rule `r11_vep_annotate_cohort`) using the offline cache, MANE-Select / canonical / biotype transcript prioritization (`--pick_allele_gene --pick_order mane_select,canonical,biotype`), and HGVS notation. Plugins are loaded from a flattened directory and included AlphaMissense, REVEL, LOFTEE (high-confidence loss-of-function flag), CADD v1.7, SpliceAI (max delta scores across acceptor/donor gain/loss), and dbNSFP v5.3.1a fields (MutationTaster_pred, PROVEAN_pred, MetaLR_pred, MetaRNN_pred, M-CAP_pred, PrimateAI_pred, ClinPred_pred, BayesDel_addAF_pred). VEP --custom annotations attached gnomAD v4.1 exome population-stratified allele frequencies and ClinVar clinical-significance fields.

#### 8. Sex inference
Genetic sex is inferred from sequencing coverage over the capture targets of the analysis-ready (post-BQSR) BAMs. For each sample, mean read depth is computed across capture-target intervals on the autosomes (chr1–chr22), chromosome X, and chromosome Y using samtools bedcov (MAPQ ≥ 20), and two normalised ratios are formed: an X-ratio (mean chrX depth / mean autosomal depth) and a Y-ratio (mean chrY depth / mean autosomal depth). Samples are called XX (X-ratio ≥ 0.80, Y-ratio < 0.15) or XY (X-ratio < 0.65, Y-ratio ≥ 0.15); samples falling outside these bands are flagged as ambiguous, capturing sex-chromosome aneuploidies, sample swaps or contamination, and low-coverage samples for manual review. Inferred genetic sex is cross-checked against clinical records, the summary table is stored in `../qc/inferred_sex.tsv`.
...
rRNA regions in the pathogen genomes are hard-masked because of the expected high homology with human rRNA.
