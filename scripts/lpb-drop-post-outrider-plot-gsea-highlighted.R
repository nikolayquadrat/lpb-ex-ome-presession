...
plotGseaTableHighlighted <- function(pathways,
    ...
    #   4. (blank, kept for layout parity with plotGseaTable's pval slot)
    #   5. padj value (textGrob)
    #   6. comma-separated highlight-gene list (textGrob)
    ...
    # `pathway_name` is needed to look up the leading-edge set.
    make_ticks_grob <- function(pathway_name, pathway_genes) {
        ...
        # Build the plot. theme_void hides axes/grids; coord_cartesian with
        # clip="off" keeps any taller-than-ylim ticks from being cut off
        # weirdly (shouldn't happen since y_lim covers max|statsAdj|*1.05).
        p <- ggplot2::ggplot() +
            ggplot2::xlim(0, n_genes) +
            ggplot2::ylim(y_lim) +
            ggplot2::theme_void() +
        ...
        # Layer 1: regular pathway-gene ticks (background, black)
        if (!is.null(df_reg)) {
        ...
        # Layer 2: leading-edge ticks (middle, configurable color). Skipped if
        # the user disabled leading-edge coloring by setting it to NULL.
        if (!is.null(df_le) && !is.null(leading_edge_color)) {
            p <- p + ggplot2::geom_segment(
                data = df_le,
                ggplot2::aes(x = rank, xend = rank, y = 0, yend = yend),
                linewidth = leading_edge_lwd, color = leading_edge_color
            )
        } else if (!is.null(df_le)) {
            # Leading-edge coloring disabled: render as regular black ticks
            p <- p + ggplot2::geom_segment(
                data = df_le,
                ggplot2::aes(x = rank, xend = rank, y = 0, yend = yend),
                linewidth = 0.3, color = "black"
        ...
        # Layer 3: highlight-gene ticks (foreground, user color). Drawn LAST
        # so they sit on top of anything else, even when packed densely.
        if (!is.null(df_hl)) {
            p <- p + ggplot2::geom_segment(
                data = df_hl,
                ggplot2::aes(x = rank, xend = rank, y = 0, yend = yend),
                linewidth = highlight_lwd, color = highlight_color
            )
        }
    ...
    for (i in seq_len(n_path)) {
        ...
        row_grobs[[i]] <- list(
            ...
            grid::textGrob(fmt_nes(fgseaRes$NES[fidx]),
                           gp = grid::gpar(fontsize = 10)),
            grid::textGrob(fmt_padj(fgseaRes$padj[fidx]),
                           gp = grid::gpar(fontsize = 10)),
