...
# Data for "size-matching" binning ======== 
exome_callable_genes_df <- read.delim(sprintf("%s/data/exome-pipe/fastq/14_callable/e1-19-combined.gene_callability.tsv", git_folder))
...
for (tier_gene in tier_genes) {
    ...
    pathway_list <- lapply(pathway_list_unfiltered, function(x) x[x != tier_gene])
    ...
    for(exp in names(experiments)) {
        ...
        for (donor in names(list_of_samples)) {
            ...
            if (tier_gene %in% gene_ranks$hgnc) {
                gene_ranks_vector <- gene_ranks_vector[names(gene_ranks_vector) != tier_gene] # exclude tier gene from GSEA calculation
            }
            list_of_ranks[[donor]] <- gene_ranks_vector
        }
        
        # **** GSEA test with the OURIDER results ========
        if (!file.exists(sprintf("%s/%s_list_of_fgsea_results.rds", output_folder, exp))) {
            ...
            saveRDS(list_of_fgsea_results, file = sprintf("%s/%s_list_of_fgsea_results.rds",
                                                          output_folder, exp))
        }
        
        # **** Aggregate and collapse the results ============
        list_of_fgsea_results <- readRDS(sprintf("%s/%s_list_of_fgsea_results.rds",
                                                 output_folder, exp))
        ...
        for (donor in names(list_of_fgsea_results)) {
            ...
            ggsave(sprintf("%s/%s_%s_emapplot_all.png",
            ...
            emapplot_collapsed   <- fgsea_emap(fgsea_res_sig, pathway_list = pathway_list,
                                               edge_source = "gene_sets", color_by = "NES", edge_cutoff = 0.05)
            ggsave(sprintf("%s/%s_%s_emapplot_collapsed.png",
                           output_folder, exp, donor),
            ...
            writexl::write_xlsx(
                list(
                    ...
                    "empirical_p_results" = empirical_p_results
                ),
                sprintf("%s/%s_%s_fgsea_results.xlsx",
                        output_folder, exp, donor))
        ...
        for (donor in names(list_of_samples)) {
            ...
            if(purrr::pluck_exists(top_gsea_plots, donor, "HALLMARK")) {
                ...
                ggsave(
                    sprintf("%s/%s_%s_gsea_hallmark_plot.png",
                            output_folder, exp, donor),
                    plot=hallmark_plot,
            ...
            if(purrr::pluck_exists(top_gsea_plots, donor, "GOBP")) {
                ...
                ggsave(
                    sprintf("%s/%s_%s_gsea_gobp_plot.png",
                            output_folder, exp, donor),
                    plot=gobp_plot,
                    width = 2000, height = 800, units = "px", bg = "white", dpi = 165)
            }
            if(purrr::pluck_exists(top_gsea_plots, donor, "KEGG")) {
                ...
                ggsave(
                    sprintf("%s/%s_%s_gsea_kegg_plot.png",
                            output_folder, exp, donor),
                    plot=kegg_plot,
                    width = 2000, height = 800, units = "px", bg = "white", dpi = 165)
            }
        ...
        saveRDS(top_gsea_plots,
                sprintf("%s/%s_list_of_gsea_plots.rds",
                        output_folder, exp))
        saveRDS(top_gsea_plots_tier,
                sprintf("%s/%s_list_of_gsea_tiered_plots.rds",
                        output_folder, exp))
...
# Aggregate =========
list_of_tables <- lapply(tier_genes, function(x) {
    readxl::read_xlsx(sprintf("%s/data/post-drop/snd-pass/%s/ba9_gtex_SZ07_fgsea_results.xlsx", git_folder, x),
                      sheet = "empirical_p_results")
})
